...
class ExcelService:    
    ...
    def create_excel_export_bookmarks():
        # Obtener solo los artículos marcados como seleccionados
        articles = Article.get_bookmarks()
        
        if not articles:
            # Si no hay artículos marcados, crear un archivo vacío con solo headers
            wb = Workbook()
            ws = wb.active
            ...
            # Agregar una fila indicando que no hay marcadores
            ...
            header_font = Font(bold=True, color='FFFFFF')
            header_fill = PatternFill(start_color='1F4E79', end_color='1F4E79', fill_type='solid')  # Azul más oscuro para marcadores
            header_alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
        ...
        # Configurar anchos de columna (mismo que el método original)
        column_widths = {
            1: 8,   # ID
            2: 25,  # Autor
            3: 20,  # Revista
        ...
        timestamp = datetime.now().strftime('%Y-%m-%d--%H-%M-%S')
        filename = f'matriz-marcadores-{timestamp}.xlsx'
